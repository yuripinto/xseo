...
class ResultsApplicationService:
    ...
    def list_pages(self, query):
        validation = validate_query_options(
            query.options,
    ...
    def list_issues(self, query):
        validation = validate_query_options(
            query.options, {"affected_url", "issue_type", "severity", "explanation"}
        )
        if not validation.success:
            return validation
    ...
    def list_duplicate_groups(self, query):
        validation = validate_query_options(
            query.options, {"content_hash", "page_count", "representative_url"}
        )
        if not validation.success:
            return validation
        return ApplicationResult.ok(
            tuple(self.read_repository.list_duplicate_groups(query))
        )
    ...
    def get_page_detail(self, query):
        ...
        if detail is None:
            return ApplicationResult.fail(
                "Page detail was not found", "page_detail.not_found"
            )
    ...
    def get_recent_crawl(self):
        ...
        if crawl is None:
            return ApplicationResult.fail(
                "No recent crawl was found", "crawl.no_recent"
            )
