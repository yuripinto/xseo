...
def test_detects_exact_duplicate_groups_by_content_hash():
    ...
    pages = (
        _page(crawl_id=crawl_id, page_id=_id(PageId, "page-3"), url=_url("https://example.com/three"), content_hash=shared_hash),
        _page(crawl_id=crawl_id, page_id=_id(PageId, "page-1"), url=_url("https://example.com/one"), content_hash=shared_hash),
        _page(crawl_id=crawl_id, page_id=_id(PageId, "page-2"), url=_url("https://example.com/two"), content_hash=ContentHash.create("hash-unique").value),
    )
